...
The limited size of the corpus available made the choice of algorithm especially crucial. The lack of noticable outliers in the visualistions at least suggest that the data is not of poor-quality. Despite the small corpus, cross-validation would still be used, and so the data was split into a training and testing set with the common 80\%-20\% split \cite{gron_2018}. 

To split the data set, simple random sampling (SRS) and DUPLEX were considered. SRS is significantly simpler to implement than DUPLEX, but for less uniformly distributed data sets, it can lead to subsets that poorly represent the input data \cite{reitermanova_2010}. The increased time complexity of DUPLEX would be an issue with larger datasets, but since this is not the case DUPLEX was choosen.
